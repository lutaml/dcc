        ...
        class InfluenceConditionsPlacement < Base
          def check_on(dcc)
            ...
            any_present = mr_list.any? do |mr|
              next false unless Dcc::TypeGuards.has_attribute?(mr,
          ...
          def has_in_descendants?(node)
            ...
            if Dcc::TypeGuards.has_attribute?(node,
                                              :influence_conditions) && !node.influence_conditions.nil?
              return true
            end

            node.class.attributes.each_key do |attr|
              value = node.public_send(attr)
              if value.is_a?(::Array) || value.is_a?(::Lutaml::Model::Serializable)
                Array(value).any? do |c|
                  has_in_descendants?(c)
                end
              end
            end
            false
