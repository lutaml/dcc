...
module Dcc::V3
  # V3 Quantity: declares DCC admin attributes + D-SI v2.x quantity element
  # references in a single `xml do` block. We avoid `include Dcc::Base::Quantity`
  # because the second `xml do` block would overwrite the base mappings.
  class Quantity < CommonElements
    ...
    # D-SI v2 quantity substitutions (Option B)
    attribute :real, ::Dcc::Si::V2::Real, collection: true
