...
## Gaps
- The six v3.4.0-rc.2 modules are absent: `measurand.rb`,
  `relative_uncertainty.rb`, `position_coordinates.rb`,
  `performance_location.rb`, `report_amended_substituted.rb`,
  `primitive_quantity.rb`. Those constants appear nowhere in `lib/`.
- The DCC v3.3 element set is complete: 44 modules ship under `lib/dcc/base/`.
...
## Files to create
One file per DCC v3.3.0 complexType. 44 files ship under `lib/dcc/base/`. Representative subset:
...
- `lib/dcc/base/used_method.rb` + `used_method_list.rb` (drafted above as `used_methods.rb`).
- `lib/dcc/base/software.rb` + `software_list.rb` (drafted above as `used_software.rb`).
- `lib/dcc/base/influence_condition.rb` + `influence_condition_list.rb`.
- `lib/dcc/base/measuring_equipment.rb`.
- `lib/dcc/base/resp_person.rb`.
- `lib/dcc/base/calibration_laboratory.rb`.
- Customer is modelled by `lib/dcc/base/contact.rb`; there is no `customer.rb`.
- `lib/dcc/base/equipment_class.rb`.
- `lib/dcc/base/location.rb`.
- `lib/dcc/base/hash_type.rb` (previousReport hash reference).
